...
<figure><img src="../.gitbook/assets/엔트리 블록 (16).png" alt=""><figcaption></figcaption></figure>
{% endtab %}
...
</strong>def compare_num(num1, num2):
    if num1 == num2:
        Entry.print("Bingo! " + try_total + " 번 만에 찾아냈군!")
        Entry.stop_code("all")
    else:
...
:1234: 8\~16번 라인의 코드가 [기존의 프로그램 코드](3.7-random.md#undefined-3)의 16\~24인을 떼어내어 compre\_num이란 사용자 함수를 만든 부분이다. 함수의 파라미터로 2개의 숫자값을 전달 받아오게 되어 함수 내부에서 그 두 수의 크기비교 결과에 따른 조건적 실행결과를 보여주게 된다. 함수를 만들고 사용하는 법을 이미 숙지했다면 전혀 이해에 어려움이 없는 부분이다.
